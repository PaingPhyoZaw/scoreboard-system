...
BEGIN
    ...
    WITH monthly_scores AS (
        SELECT 
            ...
            SUM(s.target_score) as max_possible_score  -- Using sum of target scores as the base
        FROM users u
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 
        FROM pg_constraint 
        WHERE conname = 'scores_user_id_score_date_key'
    ) THEN
        ALTER TABLE scores 
...
-- Insert sample scores with target scores
INSERT INTO scores (user_id, score_date, total_score, target_score)
SELECT 
    u.id,
    d::date,
    7 + random() * 3,  -- Random actual score between 7 and 10
    10  -- Target score is 10 for each day
FROM users u
